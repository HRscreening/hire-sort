...
export function generateStaticParams() {
  return getScreeningRubricSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { slug } = await params;
  const data = getScreeningRubricBySlug(slug);
  if (!data) return { title: 'Not found', robots: { index: false, follow: false } };

  const url = `/resources/screening-rubrics/${data.slug}`;
  const ogImageUrl = absUrl(data.meta.ogImage ?? '/logo.png');
  const ogImageAlt = data.meta.ogImageAlt ?? `${data.role} screening rubric`;
...
export default async function ScreeningRubricRoute({ params }: { params: Params }) {
  const { slug } = await params;
  const data = getScreeningRubricBySlug(slug);
  if (!data) notFound();
  ...
  const webPageJsonLd = {
    ...
    about: [{ '@type': 'Thing', name: `${data.role} screening rubric` }],
    publisher: {
      '@type': 'Organization',
      name: 'HireSort',
