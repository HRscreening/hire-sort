...
export function generateStaticParams() {
  return getInterviewQuestionsSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  const { slug } = await params;
  const data = getInterviewQuestionsBySlug(slug);
  if (!data) return { title: 'Not found', robots: { index: false, follow: false } };

  const url = `/resources/interview-questions/${data.slug}`;
  const ogImageUrl = absUrl(data.meta.ogImage ?? '/logo.png');
  const ogImageAlt = data.meta.ogImageAlt ?? `${data.role} interview questions`;
...
export default async function InterviewQuestionsRoute({ params }: { params: Params }) {
  const { slug } = await params;
  const data = getInterviewQuestionsBySlug(slug);
  if (!data) notFound();
  ...
  const webPageJsonLd = {
    ...
    about: [{ '@type': 'Thing', name: `${data.role} interview questions` }],
    publisher: {
      '@type': 'Organization',
      name: 'HireSort',
