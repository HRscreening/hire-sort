...
import { Dialog, DialogTrigger, DialogHeader, DialogDescription,DialogContent } from "@/components/ui/dialog"
...
export default function ApplyForm({ jobId }: { jobId: string }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch(`/api/screenings/${jobId}/upload-resume`, {
        method: "POST",
        body: formData,
      });
      ...
      if (!res.ok) {
        ...
        if (typeof json.detail === "string") {
          ...
          msg = json.detail.map((d: any) => d.msg || d.message).filter(Boolean).join(", ");
        } else if (typeof json.detail === "object" && json.detail !== null) {
          code = json.detail.code || json.detail.error_code || json.detail.error || "";
          msg = json.detail.message || json.detail.msg || json.detail.detail || "";
        }

        if (!code) {
          code = json.code || json.error_code || json.error || "";
        }
        if (!msg) {
          msg = json.message || json.msg || (typeof json === "string" ? json : "");
        }

        console.error("Error submitting application:", { status: res.status, code, msg, json });

        // Custom domain error messages mapping
        if (res.status === 409 || code === "APPLICATION_ALREADY_EXISTS" || code === "ALREADY_APPLIED") {
          throw new Error(msg || "An application with this email already exists for this screening.");
        } else if (code === "FILE_UPLOAD_ERROR") {
          throw new Error(msg || "An error occurred during file upload. Please try again.");
        } else if (code === "INVALID_FILE_TYPE") {
          throw new Error(msg || "Invalid file type. Only PDF and DOCX files are allowed.");
        } else if (code === "FILE_SIZE_EXCEEDED" || res.status === 413) {
          throw new Error(msg || "File size exceeded the maximum limit of 5MB.");
        } else {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-w-lg bg-ivory-light border-0 shadow-2xl rounded-2xl p-6 md:p-8 outline-none ring-0 focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0 [&_*:focus]:outline-none">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div
            ...
            className={`relative flex flex-col items-center justify-center rounded-xl border border-dashed p-5 text-center cursor-pointer transition-all duration-200 ${dragActive
